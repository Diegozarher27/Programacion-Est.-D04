- Programacion-Est.-D04
  
Nombre del estudiante: Diego Zaragoza Hernandez

Código: 222592394

Sección: D04

Profesor:JORGE ERNESTO LOPEZ ARCE DELGADO

- DESCRIPCION.
  
Repositorio de actividades y evidencias desarrolladas durante el curso de
Programación Estructurada en lenguaje C.

- TEMAS TRABAJADOS.
  
1. Fundamentos y Operadores

Operadores Relacionales: >, <, >=, <=, ==, !=

Operadores Lógicos: && (AND), || (OR), ! (NOT)

Operadores de Incremento: ++, --

2. Ciclo de Vida del Software
   
Análisis y Definición

Diseño del Algoritmo

Codificación

Pruebas y Mantenimiento

3. Estructuras de Control
   
Condicionales: if, else, switch

Ciclo for: Contador conocido

Ciclo while: Condición previa

Ciclo do while: Al menos una ejecución

Control de ciclos: break, continue

4. Arreglos y Matrices
   
Unidimensionales: Índice inicial 0

Multidimensionales: Matrices [filas][columnas]

Recorrido: Bucles for anidados

Memoria: Almacenamiento contiguo

5. Funciones
   
Modularidad: Bloques reutilizables

Paso por Valor: Copia del dato

Paso por Referencia: Uso de direcciones

Recursividad: Caso base y auto-llamada

Retorno: Uso de return

6. Punteros y Memoria
   
Operador &: Dirección de memoria

*Operador : Valor apuntado

malloc: Reserva dinámica en heap

free: Liberación de memoria

7. Estructuras y Uniones
   
struct: Tipos de datos compuestos

Acceso: Operador punto (.)

Punteros a struct: Operador flecha (->)

unions: Memoria compartida

8. Gestión de Archivos

fopen: Modos r, w, a

Validación: Puntero NULL

Operaciones: fscanf, fprintf, fgets

fclose: Cierre de flujo
